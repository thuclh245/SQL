"""Loader turning a schema manifest into catalog tables."""
...
def load_catalog_tables_from_manifest(
    database_id: str,
    manifest_path: Path,
) -> list[CatalogTable]:
    """Build catalog tables from a schema manifest describing one or more databases.

    A manifest is a JSON list of database entries, each holding ``db_id``,
    ``table_names_original``, ``column_names_original``, ``column_types``,
    ``primary_keys`` and ``foreign_keys``.
    """
    all_database_metadata = json.loads(manifest_path.read_text(encoding="utf-8"))
    if not isinstance(all_database_metadata, list):
        raise ValueError(f"Schema manifest must be a list: {manifest_path}")

    db_entry = next(
        (
            database_metadata
            for database_metadata in all_database_metadata
            if database_metadata.get("db_id") == database_id
        ),
        None,
    )
    if db_entry is None:
        raise ValueError(f"Database '{database_id}' not found in {manifest_path}")

    return _build_catalog_tables(db_id=database_id, db_entry=db_entry)
