...
function initPaneSplitter() {
  ...
  let dragging = false;
  const stopDragging = () => { if (!dragging) return; dragging = false; paneSplitter.classList.remove('is-dragging'); document.body.classList.remove('is-resizing'); };
  paneSplitter.addEventListener('pointerdown', event => { dragging = true; paneSplitter.setPointerCapture?.(event.pointerId); paneSplitter.classList.add('is-dragging'); document.body.classList.add('is-resizing'); setQuestionPaneWidth(event.clientX); });
  paneSplitter.addEventListener('pointermove', event => { if (dragging) setQuestionPaneWidth(event.clientX); });
  paneSplitter.addEventListener('pointerup', stopDragging);
  paneSplitter.addEventListener('pointercancel', stopDragging);
  paneSplitter.addEventListener('keydown', event => { if (!['ArrowLeft', 'ArrowRight'].includes(event.key)) return; const current = questionPane.getBoundingClientRect(); setQuestionPaneWidth(current.right + (event.key === 'ArrowLeft' ? -24 : 24)); event.preventDefault(); });
}
  ...
  card.innerHTML = `
    ...
    <div id="${msgId}_stepper" class="stepper-container">
      <div class="text-xs font-semibold text-slate-500 mb-2.5 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <button id="${msgId}_toggle_thinking" type="button" class="thinking-accordion-btn" onclick="toggleThinkingProcess('${msgId}')">
            ...
            <span id="${msgId}_toggle_text">Quá trình tư duy & Đồ thị (DE/DA)</span>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <div id="${msgId}_step_1" class="step-node cursor-pointer hover:border-indigo-300" onclick="toggleStepThinking('${msgId}', 1)">
          <div id="${msgId}_icon_1" class="step-icon">1</div>
          <span class="truncate">Dò tìm dữ liệu</span>
        </div>
        <div id="${msgId}_step_2" class="step-node cursor-pointer hover:border-indigo-300" onclick="toggleStepThinking('${msgId}', 2)">
          <div id="${msgId}_icon_2" class="step-icon">2</div>
          <span class="truncate">Sinh SQL & Đồ thị</span>
        </div>
        <div id="${msgId}_step_3" class="step-node cursor-pointer hover:border-indigo-300" onclick="toggleStepThinking('${msgId}', 3)">
          <div id="${msgId}_icon_3" class="step-icon">3</div>
          <span class="truncate">An toàn AST</span>
        </div>
        <div id="${msgId}_step_4" class="step-node cursor-pointer hover:border-indigo-300" onclick="toggleStepThinking('${msgId}', 4)">
      ...
      <div id="${msgId}_thinking_box" class="thinking-box hidden">
        <div class="px-3.5 py-2 bg-slate-100/80 border-b border-slate-200/80 flex items-center justify-between text-[11px] font-semibold text-slate-700">
          ...
          <span class="text-[10px] text-slate-400 font-normal">Bấm từng bước để mở/thu gọn</span>
        </div>
    ...
    <div id="${msgId}_output" class="space-y-3"></div>
  `;
...
window.toggleThinkingProcess = function (msgId) {
  ...
  const btnText = document.getElementById(`${msgId}_toggle_text`);
  if (!box) return;
  ...
  if (isHidden) {
    ...
    // Mở mặc định tất cả các bước đã có dữ liệu
    for (let i = 1; i <= 4; i++) {
...
window.toggleStepThinking = function (msgId, stepNum) {
  const box = document.getElementById(`${msgId}_thinking_box`);
  const arrow = document.getElementById(`${msgId}_toggle_arrow`);
  if (!box) return;
