...
def get_or_create_runtime(db_id: str, model_name: str | None = None) -> tuple[Any, str]:
    ...
    if db_id == "vtnet_mini":
        ...
        dataset_root = BASE_DIR / "sample data" / "synthetic" / "vtnet-mini"
        if not (dataset_root / "generated" / "vtnet.duckdb").exists():
            raise FileNotFoundError(f"DuckDB database không tồn tại trong {dataset_root}")
        runtime = VerifiedDuckDBRuntime(dataset_root, effective_model, settings)
...
def _vtnet_assets(dataset_root: Path) -> VerifiedContextAssets:
    ...
    if dataset_root not in _VTNET_ASSETS:
        _VTNET_ASSETS[dataset_root] = VerifiedContextAssets.from_dataset(dataset_root)
    return _VTNET_ASSETS[dataset_root]


class VerifiedDuckDBRuntime:
    """VTNet Mini (DuckDB) chạy qua pipeline verified-context (src/t2s/verified_context).

    Thay cho DuckDBRuntime cũ: không còn tự viết lại SQL (auto-cast, WHERE→HAVING,
    CAST date_hour); lỗi được verifier báo lại cho model để sửa.
    """

    is_verified_context = True

    def __init__(self, dataset_root: Path, effective_model: str, settings: Settings) -> None:
        provider_url = (
            os.getenv("VLLM_BASE_URL") or settings.vllm_base_url or "https://openrouter.ai/api/v1"
        )
        api_key = os.getenv("LLM_API_KEY") or settings.llm_api_key or ""
        completer = OpenAICompatibleCompleter(
            base_url=provider_url, api_key=api_key, model=effective_model
        )
